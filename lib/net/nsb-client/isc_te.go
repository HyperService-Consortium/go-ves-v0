...
// TestCreateISC is not public, just for test
func TestCreateISC(t *testing.T) {
	var opintent = iscTransactionIntent.TransactionIntent{
		Fr:  []byte{1, 2, 3},
		To:  []byte{1, 2, 3, 4},
		Seq: nsbmath.NewUint256FromHexString("00"),
		Amt: nsbmath.NewUint256FromHexString("02e0"),
	}
...
// TestUpdateTxInfo is not public, just for test
func TestUpdateTxInfo(t *testing.T) {
	...
	ret, err := NewNSBClient(host).UpdateTxInfo(&eacc{[]byte{1, 2, 3}, []byte("abcc")}, iscAddress, 0, new(iscTransactionIntent.TransactionIntent))
	if err != nil {
		t.Error("UPD ERR", err, "\n")
	} else {
...
// TestFreezeInfo is not public, just for test
...
// TestUserAck is not public, just for test
...
// TestInsuranceClaim is not public, just for test
...
// TestSettleContract is not public, just for test
